...
{
	struct send_msg_data *data = (struct send_msg_data *)arg;
...
static int get_devices_handler(struct nl_msg *n, void *arg)
{
	...
	__nfc_manager_adapter_add(idx, name, protocols, powered);
...
int __nfc_netlink_start_poll(int idx,
...
int __nfc_netlink_stop_poll(int idx)
{
	struct nl_msg *msg;
	void *hdr;
	int err;
...
int __nfc_netlink_dep_link_up(uint32_t idx, uint32_t target_idx,
...
int __nfc_netlink_dep_link_down(uint32_t idx)
{
	struct nl_msg *msg;
	void *hdr;
	int err;
...
int __nfc_netlink_adapter_enable(int idx, bool enable)
{
	struct nl_msg *msg;
	void *hdr;
	int err;
...
static int nfc_netlink_event_adapter(struct genlmsghdr *gnlh, bool add)
{
	...
	if (!attrs[NFC_ATTR_DEVICE_INDEX]) {
		print_error("Missing device index");
		return -ENODEV;
	}
	...
	if (add &&
		(!attrs[NFC_ATTR_DEVICE_NAME] ||
			!attrs[NFC_ATTR_PROTOCOLS])) {
		print_error("Missing attributes");
		return -EINVAL;
	}
	...
	if (add) {
		...
		return __nfc_manager_adapter_add(idx, name,
						protocols, powered);
	} else {
...
static int get_targets_handler(struct nl_msg *n, void *arg)
{
	...
	__nfc_adapter_add_target(adapter_idx, target_idx, protocols,
...
static int get_targets_finish_handler(struct nl_msg *n, void *arg)
{
	...
	return __nfc_adapter_get_targets_done(adapter_idx);
}
...
static int nfc_netlink_event_target_lost(struct genlmsghdr *gnlh)
{
	...
	return __nfc_adapter_remove_target(adapter_idx, target_idx);
}
...
static int nfc_netlink_event_dep_up(struct genlmsghdr *gnlh)
{
	...
	if (!attrs[NFC_ATTR_COMM_MODE] ||
		...
		print_error("Missing rf or comm modes");
		return -ENODEV;
	}
	...
	if (rf_mode == NFC_RF_INITIATOR) {
		if (!attrs[NFC_ATTR_TARGET_INDEX]) {
			print_error("Missing target index");
			return -ENODEV;
		};

		DBG("%d %d", idx, nla_get_u32(attrs[NFC_ATTR_TARGET_INDEX]));

		return __nfc_adapter_set_dep_state(idx, true);
	} else {
		return -EOPNOTSUPP;
	}
}
...
static int nfc_netlink_event_dep_down(struct genlmsghdr *gnlh)
{
	...
	__nfc_adapter_set_dep_state(idx, false);
...
static int nfc_netlink_event_tm_activated(struct genlmsghdr *gnlh)
{
	...
	return __nfc_adapter_add_device(idx, NULL, 0);
}
...
static int nfc_netlink_event_tm_deactivated(struct genlmsghdr *gnlh)
{
	...
	return __nfc_adapter_remove_device(idx);
}

static int nfc_netlink_event(struct nl_msg *n, void *arg)
{
	struct sockaddr_nl *src = nlmsg_get_src(n);
	struct genlmsghdr *gnlh = (struct genlmsghdr *)nlmsg_data(nlmsg_hdr(n));

	DBG("event 0x%x", gnlh->cmd);

	if (src->nl_pid) {
		print_error("WARNING: Wrong netlink message sender %d",
								src->nl_pid);
		return NL_SKIP;
	}
...
void __nfc_netlink_event(uv_poll_t* handle, int status, int events)
{
	struct nl_cb *cb;
	struct nlnfc_state *state = (struct nlnfc_state *)handle->data;
	...
	nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, nfc_netlink_event, state);
...
static int nfc_event_listener(struct nlnfc_state *state)
{
    ...
    handle->data = state;
    uv_poll_init(uv_default_loop(), handle, sock);
    uv_poll_start(handle, UV_READABLE | UV_DISCONNECT, __nfc_netlink_event);
...
#define nla_for_each_nested2(pos, nla, rem) \
...
static int family_handler(struct nl_msg *msg, void *arg)
{
	...
	nla_for_each_nested2(mcgrp, tb[CTRL_ATTR_MCAST_GROUPS], rem_mcgrp) {
		struct nlattr *tb_mcgrp[CTRL_ATTR_MCAST_GRP_MAX + 1];

		nla_parse(tb_mcgrp, CTRL_ATTR_MCAST_GRP_MAX,
			  (nlattr *)nla_data(mcgrp), nla_len(mcgrp), NULL);
...
int __nfc_netlink_init(void)
{
	...
	nfc_state = (nlnfc_state*)malloc(sizeof(struct nlnfc_state));
	if (!nfc_state)
		return -ENOMEM;

	nfc_state->cmd_sock = nl_socket_alloc();
	if (!nfc_state->cmd_sock) {
		print_error("Failed to allocate NFC command netlink socket");
		err = -ENOMEM;
		goto state_free;
	}
	...
	if (!nfc_state->event_sock) {
		print_error("Failed to allocate NFC event netlink socket");
		err = -ENOMEM;
	...
	if (err) {
		print_error("Error adding nl event socket to membership");
		goto handle_event_destroy;
	}
